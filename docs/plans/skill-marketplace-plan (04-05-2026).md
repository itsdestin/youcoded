...
**Goal:** Transform YouCoded's static command menu into a full skill marketplace with discovery, management, sharing, and customizable quick chips.
...
**Source repos:**
- Desktop UI source: `~/youcoded-core/desktop/src/`
- Android app: `~/youcoded/app/src/main/`
- Built web assets: `~/youcoded/app/src/main/assets/web/` (generated by `scripts/build-web-ui.sh`)
...
**Files:**
- Modify: `~/youcoded-core/desktop/src/shared/types.ts:76-84`
...
```typescript
function addSkill(id: string, fallbackName: string, fallbackDesc: string, inferredSource: 'youcoded-core' | 'self' | 'plugin', pluginName?: string) {
...
Run: `cd ~/youcoded-core/desktop && npm run build 2>&1 | tail -20`
...
**Files:**
- Create: `~/youcoded-core/desktop/src/main/skill-config-store.ts`

Reads and writes `~/.claude/youcoded-skills.json`. Handles migration from the current hardcoded state.
...
Run: `cd ~/youcoded-core/desktop && npx tsc --noEmit 2>&1 | head -20`
...
**Files:**
- Create: `~/youcoded-core/desktop/src/main/skill-provider.ts`
- Create: `~/youcoded-core/desktop/src/main/skill-share.ts`
...
const CACHE_DIR = path.join(os.homedir(), '.claude', 'wecoded-marketplace-cache');
...
const REGISTRY_BASE = 'https://raw.githubusercontent.com/anthropics/wecoded-marketplace/main';
...
**Files:**
- Modify: `~/youcoded-core/desktop/src/main/preload.ts`
- Modify: `~/youcoded-core/desktop/src/main/ipc-handlers.ts`
- Modify: `~/youcoded-core/desktop/src/main/remote-server.ts`
- Modify: `~/youcoded-core/desktop/src/renderer/hooks/useIpc.ts`
- Modify: `~/youcoded-core/desktop/src/renderer/remote-shim.ts`
...
Run: `grep -rn "registerIpcHandlers" ~/youcoded-core/desktop/src/main/ --include="*.ts"` to find the call site.
...
**Files:**
- Create: `~/youcoded-core/desktop/src/renderer/state/skill-context.tsx`
...
**Files:**
- Modify: `~/youcoded-core/desktop/src/renderer/components/CommandDrawer.tsx`
- Modify: `~/youcoded-core/desktop/src/renderer/components/QuickChips.tsx`
- Modify: `~/youcoded-core/desktop/src/renderer/App.tsx`
...
**Files:**
- Create: `~/youcoded-core/desktop/src/renderer/components/Marketplace.tsx`
- Create: `~/youcoded-core/desktop/src/renderer/components/SkillDetail.tsx`
- Modify: `~/youcoded-core/desktop/src/renderer/components/SkillCard.tsx`
...
**Files:**
- Create: `~/youcoded-core/desktop/src/renderer/components/SkillManager.tsx`
- Create: `~/youcoded-core/desktop/src/renderer/components/SkillEditor.tsx`
- Create: `~/youcoded-core/desktop/src/renderer/components/ShareSheet.tsx`
- Create: `~/youcoded-core/desktop/src/renderer/components/CreatePromptSheet.tsx`
...
**Files:**
- Create: `~/youcoded/app/src/main/kotlin/com/destin/code/skills/SkillConfigStore.kt`
- Create: `~/youcoded/app/src/main/kotlin/com/destin/code/skills/SkillScanner.kt`
- Create: `~/youcoded/app/src/main/kotlin/com/destin/code/skills/MarketplaceFetcher.kt`
- Create: `~/youcoded/app/src/main/kotlin/com/destin/code/skills/LocalSkillProvider.kt`
- Create: `~/youcoded/app/src/main/kotlin/com/destin/code/skills/SkillShareCodec.kt`
- Modify: `~/youcoded/app/src/main/kotlin/com/destin/code/runtime/SessionService.kt`

Port the desktop's `LocalSkillProvider` to Kotlin for identical Android behavior.

- [ ] **Step 1: Create SkillConfigStore.kt**

Kotlin port of the TypeScript config store. Reads/writes `~/.claude/youcoded-skills.json` using `org.json` (already available in Android). Same schema, same migration logic. Must use atomic write-to-temp-then-rename pattern matching the desktop implementation. Must back up corrupt config to `.bak` before resetting.
...
**IMPORTANT:** The home directory path MUST come from `Bootstrap.getHomePath()` — NOT `Environment.getExternalStorageDirectory()`. Per the project's CLAUDE.md, all paths use the relocated Termux prefix at `context.filesDir`, not external storage. The config file path is `${Bootstrap.getHomePath()}/.claude/youcoded-skills.json`.
...
Base64url encode/decode for `youcoded://skill/` and `youcoded://plugin/` URLs. Uses `android.util.Base64` with `URL_SAFE | NO_WRAP` flags.
...
Run: `cd ~/youcoded && ./gradlew assembleDebug 2>&1 | tail -20`
...
**Files:**
- Modify: `~/youcoded/app/src/main/AndroidManifest.xml`
- Modify: `~/youcoded/app/src/main/kotlin/com/destin/code/MainActivity.kt`

Register the `youcoded://` URL scheme and route deep links to the import flow.
...
Test with: `adb shell am start -a android.intent.action.VIEW -d "youcoded://skill/eyJ2IjoxLCJ0eXBlIjoicHJvbXB0In0"`
...
git commit -m "feat: register youcoded:// deep link scheme for skill imports"
```
...
**Files:**
- Run: `~/youcoded/scripts/build-web-ui.sh ~/youcoded-core`

Rebuild the React UI from the desktop source and verify the full flow on both platforms.

- [ ] **Step 1: Build web UI**

Run: `cd ~/youcoded && bash scripts/build-web-ui.sh ~/youcoded-core`
...
**Files:**
- Create: new repository `wecoded-marketplace` (or directory within existing repo)
...
```bash
mkdir -p ~/wecoded-marketplace/registry/prompts
mkdir -p ~/wecoded-marketplace/registry/plugins
mkdir -p ~/wecoded-marketplace/.github/workflows
```
...
**Files:**
- Review: `~/youcoded/docs/specs/skill-marketplace-design (04-05-2026).md`
...
2. Reference the visual mockups at `youcoded/.superpowers/brainstorm/49790-1775375392/content/marketplace-hybrid-v2.html`
