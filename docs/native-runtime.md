...
- **`quiesce(id)` (M2) is a SEPARATE, STRONGER teardown than `interrupt()` — cross-device takeover only, never the Stop button.** `interrupt()` aborts only the in-flight turn and leaves the M1 send queue draining (a queued message starts a new turn right after). `quiesce()` additionally clears the queue synchronously, awaits one macrotask (lets a same-tick `send()` finish its deferred dispatch before the abort), and awaits the turn settling + append chain — its postcondition is "no further appends", which a cross-device transcript flush depends on. From its first line until the session is destroyed, `send()`, `/clear`, `/compact`, `/skill` and idle report delivery are refused (`not-live`) — the takeover's flush and lease release come after `quiesce()` returns; `endQuiesce()` lifts the refusal only for a handoff that stopped BEFORE the lease release — after it the conversation belongs to another device, so a session that failed to destroy keeps refusing. `createHolderTakeover` (conversations/takeover.ts) branches to it for a native holder instead of sending the ESC byte. Depth + full 5-step order: `docs/conversations.md` → "Native provider participation".
...
- **Permission precedence:** tool-layer guards (secret paths, `external_directory`) sit BELOW all configuration and never yield; the destructive deny-list is CONFIG — an explicit remembered Always-allow beats it (by design, consequence-gated in UI, surfaced via the `denyListed` flag on the ask). Guard: `permission-engine.test.ts`. **Two Bash floors sit below the remembered grants** (2026-09-23, `harness-session.ts` step 3b). Each only turns an allow or ask into a forced ask — an explicit DENY rule still denies (`floorStop && configured.action !== 'deny'`). The first: the removal-target check (`tools/rm-target.ts`) forces an ask — never a deny, never an allow — for a Bash `rm` whose target is the workspace, the home folder, the disk root, a system folder, or everything inside one of those. It reads `rm` after shell keywords (`then`, `do`, `{`, `!`…) and wrappers (`sudo`, `timeout 5`, `env X=1`, `xargs`…), reads `$(…)`/backticks as commands too, and asks when a recursive target is a command's output. A BARE variable (`rm "$f"`) never asks — empty, it removes nothing — and `${X:?}` is never empty; text after an unguarded variable is judged as if it were empty, and the reason says "if $X is empty". Both floors share one tokenizer (`tools/shell-words.ts`), which also ends a command at an unquoted `#` comment, reads `sh/bash -c` (incl. `-c --`), `sudo bash -c` and `eval` scripts as command lines, splits heredoc bodies out as TEXT unless a shell (`bash`, `sh`, `ssh host`, …) runs them, and matches wrappers by basename (`/usr/bin/sudo`). The removal floor also reads `${HOME:?}`, `$PWD`, `--rec` and trailing slashes, and treats `cd "$tmp"` after `tmp=$(mktemp …)` as a known temp folder. Guard: `shell-words.test.ts` runs a 76-command everyday sweep through both floors. A second floor works the same way for secrets (Destin, 2026-09-23): a Bash command that names a path the file tools refuse — `~/.ssh`, `.env`/`.envrc`, `~/.aws`, `.netrc`, `~/.config/gh`, the home credential files — always gets a card (`tools/bash-secret-paths.ts`). It reuses `checkPathGuard`'s own deny verdict, so there is ONE secret list; never add a second. It asks when a file is READ (also: curl `@file`/`-F f=@file`/`--upload-file`, `git show rev:path`, `dd if=`, interpreter one-liners and heredoc scripts naming a secret literal, globs that could expand to one — `cat .env*` — and `find | xargs` where the find could match one): arguments, `--flag=value` values, assignments, input redirects (`cat <.env`, `$(<.env)`). It does not ask for a file that is only WRITTEN (`> .env`, `>> .gitignore`, `tee`/`touch`, the destination of `cp`/`mv`), for text (`echo`, `-m "…"`, `--exclude=`, `find -name`, heredoc bodies), or for dotenv templates (`.env.example`/`.sample`/`.template`/`.dist` — the Bash check's exemption only). A word that only resembles a secret (`printenv`, `env:check`, `.envrc-template`) never matches. Commands that only check existence or show a name, size or count are quiet — an explicit allowlist (`ls`, `dir`, `test`, `[`, `[[`, `stat`, `wc`, `du`), judged per simple command, so `[ -f .env ] || cp .env.example .env` is quiet while `ls .env && cat .env` still asks; anything not on the list falls toward asking. A command another command RUNS is judged like a top-level one: each `find -exec/-execdir/-ok/-okdir` command, with `{}` standing for a secret the find's filters could match (none for `-name '*.ts'`; no filter or `-regex` counts as could-match, falling toward asking); `sh/bash -c` scripts; and `xargs`/`parallel` when the command on the left of the pipe names a secret (`find . -name .env | xargs cat`, `echo .env | xargs cat`). A `find` that runs nothing only lists names and is quiet, like `ls`; `-fprint*` only writes the listing. Outside Full auto the card adds one line picked from the floor's KIND (`FloorStop`), so it never claims more than the check knows: "this deletes a protected folder" / "this could delete a protected folder if a variable in it is empty" / "which folder this deletes can't be known in advance" / "this uses a file that holds passwords or keys" / "this could read a file that holds passwords or keys" (`deny-list-copy.ts` `floorAskNote`). grep's first plain argument is its pattern, and `git add`/`rm`/`check-ignore` only name a file (quiet); deleting a file never reads it. Both floors' asks hide "Always allow" (`floorStop` on the ask — one of five kinds, `'removal' | 'removal-if-empty' | 'removal-unknown' | 'secret-path' | 'secret-maybe'`, `shared/types.ts` `FloorStop`), nothing is remembered from them, Full auto shows its stop band (worded by the floor when the deny-list has no family), and a helper's floor ask goes to the person. Guards: `rm-target.test.ts`, `bash-secret-paths.test.ts`, `harness-session-loop.test.ts` ("removal-target floor").
- **`external_directory` costs a card for WRITES ONLY** (2026-09-05). A path outside the session cwd still returns the `'external'` verdict, but `harness-session.ts` charges it only to Write/Edit; Read/Grep/Glob run with no ask in every permission mode, Ask First included. *Why:* reading changes nothing, and Bash could already `cat` the same bytes with no card (see "The Bash tool bypasses…" below; since 2026-09-23 only a SECRET path gets a card there) — the prompt only ever taxed the polite tools. Secret-path hard-denies are unaffected and still absolute. Set: `READ_ONLY_PATH_TOOLS`. Guard: `harness-session-loop.test.ts` → "reads outside the workspace never ask".
...
- **The Bash tool bypasses the file-tool guards' HARD refusal** — secret-path denial and the cwd jail live in the file tools. Since 2026-09-23 a Bash command that names a secret path is ASKED about every time (the secret-path floor above), but a command that builds the path at run time (variables, base64, a script) still defeats it, and the command-glob deny-list can't catch every phrasing. The guards are honest friction, not a sandbox. Don't present them as a security boundary, and don't try to glob your way to one. (Migrated from workspace `docs/PITFALLS.md`.)
- **Grep and Glob stop after 180 s; the turn keeps going.** Each declares `caps.timeoutMs: SEARCH_TIMEOUT_MS` (`tools/registry.ts`, 180 000). `defineTool` gives such a tool a DERIVED signal (`combineSignals`) that aborts when the turn aborts OR the deadline fires — never the other way round, because the driver reads an aborted TURN signal as a user interrupt. On expiry the call returns an error result ("timed out after 180s and was stopped … narrow it") the model can act on. `ctx.toolTimeoutMs` is test-only and only shortens a deadline a tool already declares. *Why:* on 2026-08-26 one Grep from a home-folder conversation sat four hours inside a network-mounted drive until Stop.
<!-- verify: {"path": "youcoded/desktop/src/main/harness/tools/registry.ts", "contains": "SEARCH_TIMEOUT_MS = 180_000"} -->
...
Makes the harness usable on locally-hosted models: capability profiles, real context-window enforcement, grammar-constrained tool args, per-tier prompt steering, near-limit single-summary compaction, and live StatusBar chips. Still gated on `native.supported`.
...
- **Near-limit compaction is one validated summary, never prune-first.** `planContextBudget` reserves a fixed reply and margin against the real model window; the loop estimates the NEXT request from measured provider input plus newly appended messages/tools, not the previous step alone. `selectCompactionCut` retires whole user-delimited groups and keeps a bounded recent tail, never an orphan tool result. Only the summarizer's copy of oversized retired tool output can be shortened; the accepted history and visible transcript are unmodified on any failed summary. The summary call uses the conversation's system/tools prefix, a separate output allowance and the full handoff prompt; only a complete, nonempty, fitting candidate replaces history. A 60-second inactivity watchdog (five minutes before first activity) and Stop abort it without committing partial output. A same-history failure fence prevents repeated summaries; new input may retry. `fitToContext` is reserved for an old rebuilt reopen whose summary input cannot fit, and is never a normal moving-front trimmer. A recognized adapter-specific structured context overflow can trigger one tighter compaction and one rejected-request retry before any output; generic errors, auth, quota and network failures cannot. The summary's usage is billed once on `compact-summary`, not folded into the turn. Guards: `compaction-budget.test.ts`, `harness-compaction.test.ts`, `harness-session-loop.test.ts`, `prune-gate.test.ts`.
- **Automatic compaction is a rewrite inside the same turn.** `compact-summary` carries `{summary, autoCompaction:true}`; the renderer inserts one UUID-stable marker without ending the live turn or its running tools, and bills the summary once. Manual `/compact` and Claude Code still end their turns. A native compaction card is ended by the call's own answer (`compactionPending.awaitsResult`), never App's 3-minute watchdog, which lost the marker of slow local summaries; a Stop drops the card with no marker (`COMPACTION_CANCELLED`). Guards: `context-gauge-after-rewrite.test.ts`, `remote-snapshot-cut-line.test.tsx`, `native-session-host-continuation.test.ts`.
- **The saved compaction record is the `compact-summary` line itself** (`compactionRecord`: `v`, `generation`, `sourceRevision`, `sourceDigest`, `resumeFrom`, `coveredThrough`; `NativeSessionHost.commitCompaction`). The harness adopts the rewrite only after that line is appended; a stale revision, generation, identity or failed flush rejects the candidate. Reopen (`restorePortableHistory` in `history-rebuild.ts`) rebuilds summary + every valid event from `resumeFrom`, across prompt, date, tool or model changes; a malformed or shifted record falls back to the previous valid one, else the raw transcript. Guards: `harness-history-rebuild.test.ts`, `accepted-history-store.test.ts`, `native-session-host-continuation.test.ts`.
- **Only summarized messages fade.** The marker lands AFTER the kept tail, so "dim everything above the marker" faded messages the model still sees. `compact-summary` carries `retainedFromUuid` (the user message or `/skill` event opening the kept turn, `HarnessSession.retainedTurnStart`); `findArchiveBoundary` moves the fade line to it. Unknown or paged-out → this marker dims nothing new (the safe side). Guard: `archive-boundary.test.ts`.
- **A model switch is fit-checked (U11).** The picker calls `native:switch-model`; `NativeSessionHost.switchModel` measures the chat against the NEW model's budget (`HarnessSession.fitForWindow`). Fits → the same swap as `setBinding`. Too long → `needs-summary`, nothing changes, and `ModelSwitchPrompt` asks: Summarize and switch runs `compactNow(undefined, newWindow)` on the CURRENT model (tail and post-check sized for the new one) and switches only if it commits; X/Esc (or Stop while summarizing) stays. An unknown new window never blocks. Guards: `native-switch-model.test.ts`, `ModelSwitchPrompt.test.tsx`, `ipc-channels.test.ts`.
...
- **ChatGPT-plan requests carry the session id as the `session-id` header, not only as `prompt_cache_key`** (`chatgpt-model.ts` `transformParams`; `x-client-request-id` mirrors it). The Codex backend picks the server — and so the cache — from that header (codex-rs: "ChatGPT derives cache affinity from the Responses session-id header"); Codex, OpenCode, pi and Hermes all send it. Without it, follow-up turns hit 13/20 vs OpenCode's 18/20; with it 18/20, and 45/45 across a restart + resume + compaction (live rig, workspace `docs/active/investigations/2026-09-23-chatgpt-cache-affinity.md`). Cache lifetime on GPT-5.6+ is a fixed 30-minute sliding window: never send `prompt_cache_retention` (the backend 400s it on these models). Guard: `chatgpt-model.test.ts`.
- **The near-limit window budget** (`planContextBudget` in `compaction.ts`) uses `C = min(real window, provider input limit)`, fixed request cost `F` (system and tool schemas), summary-instruction overhead `J`, reply reserve `min(16,000, floor(C/4))` on a known window, margin `max(min(1024, floor(C/32)), floor(C/100))`, and summary allowance `S = min(13,107, floor((C − F)/4))` (further clamped by a known provider summary-output limit). Trigger is `C − max(reply reserve, S) − margin − J`; the token tail target is `min(20,000, floor((trigger − F)/4))`, rounded down to a complete group. The output cap on the outgoing request is recomputed from the SAME anchored occupancy that made the compaction decision. An unfit newest group produces an explicit error instead of silently front-trimming accepted history. The older `contextBudget` and its pair-aware `fitToContext` remain for legacy-reopen fallback and decoding older snapshots, not the ordinary loop. Guard: `compaction-budget.test.ts`, `message-size.test.ts`, `harness-session-loop.test.ts`.
- **The summary request reuses the conversation's warm prefix** (`generateSummary`): the same system text, tools and adapted retired span, followed by a single summary instruction and `toolChoice: 'none'`. Which user-role messages the app wrote (earlier summaries, notices, rules, skills) is said in a note appended to the instruction (`summaryProvenanceNote`), never by editing those messages — an edited early message broke the cache match for every compaction after the first. For an oversized retired tool result it shortens only a deep copy sent to the summarizer; it never front-trims user input, the retained tail, or accepted history. The summary output allowance, margin and instruction framing are reserved before sending. Anthropic's SDK drops tools under `'none'`, so its summary call cannot reuse the tool-schema prefix. Guards: `summary-warm-prefix.test.ts`, `compaction.test.ts`.
- **`usage.expectedRebuild` on `turn-complete`** (`harness-session.ts` `prefixMoved`) marks a deliberate summary (auto/manual), clear, or real model swap; unchanged appends never set it. Prune-only commits no longer happen, though old pruned snapshots can still decode. Low cache reads with the flag are an expected prefix rebuild; without it they need investigation. Guard: `cache-rebuild-flag.test.ts`.
...
- **The `<env>` snapshot is the LAST part of the system prompt** (`prompt-assembly.ts`; `ENV_OPEN` marks it). Its date and git line differ between sessions started minutes apart, so everything before it — identity, preset, project instructions, doctrine — is what a new conversation in the same folder can reuse. Direct Anthropic splits the system prompt at `ENV_OPEN` and marks the stable part as its own block (3 of 4 markers), because Anthropic matches only at marked block boundaries. Guards: `prompt-assembly.test.ts` (order), `prompt-cache.test.ts` (split).
- **Parked, with reasons** (survey): llama.cpp `--cache-reuse` (probed 2026-09-23 on b10665: Qwen 3.5 (hybrid) and Gemma 4 (SWA) both log "cache_reuse is not supported by this context, it will be disabled" — that is 10 of the 11 curated local models, gpt-oss is SWA too; revisit only for a plain-attention model), `--no-context-shift` and `--cache-ram` (already the pinned build's defaults), `--slot-save-path` (writes chat content to disk for an idle-unload case not addressed), Anthropic's cache-diagnosis beta and a cross-provider prefix-diff observer (the flag above already makes regressions visible).

- **Luna rig hooks — experiment-only, inert unless fully opted in** (`providers/luna-request-guard.ts`, `luna-browser-open.ts`, `lunaPathRefused` in `harness/tools/guards.ts`, the `beforeModelRequest` hook in `chatgpt-auth.ts`, `setExperimentKwinDisabled`). They serve the live cache-measurement rig in the youcoded-dev workspace repo (`scripts/luna/README.md` there — the quota gate server and runners live in that repo, not this one): every ChatGPT model send reserves a slot from a loopback quota gate first, file/Bash tools are jailed to the fixture, web tools refuse, and the KWin helper and machine-row registration are skipped. Active ONLY when `YOUCODED_LUNA_EXPERIMENT=1` AND the dev profile is exactly `luna-eval` AND the app is not packaged; `main.ts` deletes the variable otherwise, so the tool jails (which read it at call time) can never fire in an installed app, and an installed app ignores the variable instead of refusing to launch. Guards: `chatgpt-auth.test.ts` (experiment request guard block), `harness-tool-guards.test.ts`, `kwin-helper.test.ts`.
...
- **A repeat `Skill` call gets "already loaded", not the body again** (2026-09-23; transcripts showed one GPT session loading one skill 33 times). The session's `servedSkills` set holds resolved ids from the Skill tool AND the user's `/skill-name` (`runSkill`), and is cleared at exactly the sites `servedReads` is — resume, `/clear`, prune, summarize — so the reply never vouches for a body that was cut; after a cut the next call delivers it again. For the same reason only a WHOLE delivery is remembered: a body over the tool's `maxChars` cap, or a user `/skill-name` body cut by `fitInjection` (`runSkill`'s `cut`), is not. The reply is not `isError`, because models retry errors. The tool description also says when to load a skill and not to reload one, and each listed description is capped by `clipDescription` (first 300 + last 200 chars — trigger phrases usually sit at the end). Guards: `native-tools-polish-skill.test.ts`, `native-tools-polish-session.test.ts`, `skill-invocation-display.test.ts`.
...
- **Skill** takes only `skill` — its `args` field was REMOVED 2026-09-23: GPT models filled it with status notes on most calls, and the model already has the user's words. The user's own `/skill-name words` keeps its arguments (`skill-invocation.ts`).
...
- The plugin-manifest scan (the pre-existing `~/.claude/plugins/*/mcp-manifest.json` walk that
  ...
  manifest schema (`platform`, and since 2026-09-23 a `platforms` list that wins over it; `auto`,
  `command_windows`, `{{plugin_root}}` template expansion, with `${PACKAGE_DIR}` as an alias)
