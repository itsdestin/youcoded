...
- **Automatic compaction is a rewrite inside the same turn.** `compact-summary` carries `{summary, autoCompaction:true}`; the renderer inserts one UUID-stable marker without ending the live turn or its running tools, and bills the summary once. Manual `/compact` and Claude Code still end their turns. Release 1 is in-memory: an exact accepted-history sidecar can restore a summary only while its original transcript, prompt and binding still match; otherwise reopen may reconstruct pre-compaction history. Release 2's portable commit and restore are not yet implemented. Guards: `context-gauge-after-rewrite.test.ts`, `remote-snapshot-cut-line.test.tsx`, `native-session-host-continuation.test.ts`.
