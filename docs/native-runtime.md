...
- **The summary request reuses the conversation's warm prefix** (`generateSummary`): the same system text, tools and adapted retired span, followed by a single summary instruction and `toolChoice: 'none'`. For an oversized retired tool result it shortens only a deep copy sent to the summarizer; it never front-trims user input, the retained tail, or accepted history. The summary output allowance, margin and instruction framing are reserved before sending. Anthropic's SDK drops tools under `'none'`, so its summary call cannot reuse the tool-schema prefix. Guards: `summary-warm-prefix.test.ts`, `compaction.test.ts`.
