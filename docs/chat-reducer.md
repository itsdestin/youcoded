...
`SESSION_PROCESS_EXITED` spreads `endTurn()` and then overrides `attentionState: 'session-died'` — one of three cases where the post-endTurn attention is not `'ok'`. The second is `NATIVE_SESSION_ERROR` (native runtime, PR Plan A), which spreads `endTurn()` then overrides `attentionState: 'error'` + `errorMessage`. The third (2026-09-16, specialists plans) is `TRANSCRIPT_REPLAY_COMPLETE`: when the replayed session already shows `error` or `session-died`, it re-asserts that `attentionState` and `errorMessage` after `endTurn()` (a `stuck`/`stalled` state belongs to the turn being ended and still clears). All three follow the same spread-then-override pattern.
...
- **`shared-fixtures/attention-classifier/` is the contract** — adding a `BufferClass` or tweaking a `classifyBuffer` regex requires a fixture change in the same commit; the parity-fixtures section of `attention-classifier.test.ts` enforces it.
