...
### Typed tab = Tab key (dropped from the message)
- **Files:** `desktop/src/renderer/components/outgoing-message.ts` (tabs → spaces), `desktop/src/renderer/state/chat-reducer.ts` (`sameUserMessage` / `visibleText`)
- **Depends on:** CC's input bar treating a `\t` byte written to the PTY as the Tab KEY, not text — it is dropped from the submitted message (verified CC 2.1.x, 2026-09-23: "reply ok\tTABMARK" was submitted as "reply okTABMARK", with the "alt+t to toggle thinking" hint shown). The chat therefore never writes a tab; the reducer also ignores spacing when confirming a bubble, in case CC normalises other whitespace.
- **Break symptom:** A message pasted from a table shows twice — the recorded copy at the top, the unconfirmed bubble pinned at the bottom with replies appearing above it.
