...
### `CLAUDE_PID` in every hook's environment (nested-claude gate)
- **Files:** `desktop/hook-scripts/relay.js`, `desktop/hook-scripts/relay-blocking.js` (forward it as `_claude_pid`), `desktop/src/main/hook-relay.ts` (`HookOwnerGate`); Android `app/src/main/assets/hook-relay.js`, `hook-relay-blocking.js` (as `claudePid`), `app/src/main/kotlin/com/youcoded/app/parser/EventBridge.kt` (`HookOwnerGate`).
- **Depends on:** CC setting `CLAUDE_PID=<its own process id>` in the environment of every hook command (and Bash-tool child) — read out of the 2.1.281 binary on 2026-09-23 (`xHe()` builds `{CLAUDECODE, CLAUDE_CODE_SESSION_ID, CLAUDE_CODE_CHILD_SESSION, CLAUDE_CODE_SESSION_ATTENDED, CLAUDE_PID}` for hooks). A nested `claude` overwrites the inherited value with its own pid, which is what tells the app its hooks are not the session's. **Android's pinned CC 2.1.112 does NOT set `CLAUDE_PID`** (checked in its npm `cli.js`, 2026-09-23), so the Android gate is inert — fails open — until that pin moves. The pid of the session's first `SessionStart` owns it (or, if a pid already sent tool hooks before any SessionStart, that earlier pid — the real SessionStart was lost) (Android registers SessionStart for this since 2026-09-23); any other pid is ignored once an owner exists (a blocking PermissionRequest is ended with no decision, so the nested process falls back to its own prompt). Missing pid fails OPEN.
- **Break symptom:** If CC stops setting `CLAUDE_PID`, the gate silently turns off and a nested `claude` can again remap a chat view or raise a permission card in the parent session (the 2026-07-26 signature). If CC ever set it to something other than the hook-running process (e.g. the parent's pid for a nested run), nested hooks would pass the gate the same way. If CC stops firing SessionStart at launch, no owner is ever claimed and the gate fails open. If CC ever ran a session's hooks from more than one process (e.g. out-of-process teammates inside the PTY), those extra processes' hooks would be ignored. Detection: `desktop.log` grep for `ignoring hooks from a nested claude process`.
