...
class Bootstrap(internal val context: Context) {
    ...
    fun installHooks() {
        ...
        // "skip if YouCoded is installed" branch was load-bearing when the
        ...
        val isOurStatusline = currentStatuslineCmd.isEmpty()
                ...
                || currentStatuslineCmd.contains("youcoded-core")
                || currentStatuslineCmd.contains("youcoded")
    ...
    /**
     * Deploy the Auto-Title instruction into CLAUDE.md if YouCoded is not installed.
     */
    private fun deployAutoTitleInstruction() {
        val claudeMd = File(homeDir, ".claude/CLAUDE.md")
        val marker = "## Auto-Title"
        val instruction = """
## Auto-Title
...
""".trimStart()
        ...
        if (claudeMd.exists()) {
            ...
            if (content.contains(marker)) return // Already present
            claudeMd.appendText("\n$instruction")
        } else {
            claudeMd.parentFile?.mkdirs()
            claudeMd.writeText("# CLAUDE.md\n\n$instruction")
        }
    }
