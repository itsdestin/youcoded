...
import type { TranscriptEvent, InjectedMeta, PlanView, FloorStop } from '../../shared/types';
...
// §1): the module is deleted along with plan-child mode.
...
export interface HarnessSessionOpts {
  ...
  /** Resolved provider type. WHY retained beside the profile: plan eligibility
  ...
  // WHY plan-child mode is GONE (spending rework stage 1, design §1/§3):
  // `planChild` (a `PlanChildRequestGate`, budget-adapter.ts — deleted) used
  // to switch a plan specialist's session into a separate reservation-bound
...
interface StepResult {
  ...
  /** propose_plan calls announced through tool-input-start. WHY carried out of
  ...
  // WHY no reportedTokens any more (spending rework stage 1): it existed to
  // tell the deleted plan-child budget gate's settle() an AUTHORITATIVE
  // usage figure from an ESTIMATED one. `afterReply` (T2) reads `usage`
  // directly instead.
...
// WHY PlanBudgetStopError/PLAN_BUDGET_EXHAUSTED_STOP_REASON are GONE
// (spending rework stage 1, design §3): a stop reason still exists for a
// plan hitting its spend limit — renamed `plan_limit_reached` — but T2 owns
// wiring it through the ordinary request path's `beforeRequest` hook; there
// is no separate plan-child request driver left to throw a budget error.

// WHY withoutContinuationParts/planWireTools are GONE (spending rework stage
// 1, design §1): both served the deleted plan-child request driver
// (planWireMessages/planSetupRequest/planNextRequestBound/runPlanStep) —
// stripping provider-private continuation parts and describing the exact
// tool list for a budget adapter that no longer exists.
...
export class HarnessSession extends EventEmitter {
  ...
  /** Whether images may reach the model. WHY no plan-child exclusion any more
   *  (spending rework stage 1, Revision 1 D8): "with nothing bounded in
   *  advance, plan specialists can see images like any other specialist" —
   *  an intended consequence of the rework, not a regression. */
  private get imagesAllowed(): boolean { return this.profile.supportsVision; }
  ...
  private pairWritingPlans(text: string, stopped: boolean, assistantText: string): ToolCall[] {
    ...
    if (calls.length > 0) {
      // Each closed call's tool-result event was already recorded by
      // terminateWritingPlans, independently of these two positions — a precise
      // per-message origin isn't available here. Fail closed (null) rather than
      // publish a checkpoint that omits a real uuid.
  ...
  async compactNow(focus?: string, targetContextLength?: number): Promise<{ ok: true } | { ok: false; reason: 'turn-in-flight' | 'nothing-to-compact' | 'summary-failed' | 'interrupted' | 'cannot-fit' }> {
    ...
    // WHY no plan-child refusal any more (spending rework stage 1, design
    // §3): plan children now compact like any other session — "Plan
    // children now use the ordinary request path: … compaction". A summary
    // call's usage reaches `afterReply` (T2) exactly like an ordinary reply.
  ...
  async send(text: string, attachments: string[] = [], opts?: { historyNote?: string; toolsDisabled?: boolean }): Promise<void> {
    ...
    return this.beginTurn(text, () => this.emitEvent('user-message', attachments.length ? { text, attachments } : { text }), attachments,
      { historyNote: opts?.historyNote, toolsDisabled: opts?.toolsDisabled === true });
  }
   ...
   *  on resume; it only has to steer the turn it rides. `appGenerated` marks
   *  the entering message itself as app-authored (runSkill/runNotice), not
   *  typed by the human — kept as a named option alongside historyNote/
   *  toolsDisabled rather than another positional boolean. */
  private async beginTurn(text: string, emit: () => string, attachments: string[] = [],
    opts?: { historyNote?: string; toolsDisabled?: boolean; appGenerated?: boolean }): Promise<void> {
    const { historyNote, toolsDisabled = false, appGenerated = false } = opts ?? {};
    ...
    if (historyNote) {
      ...
      this.historyOrigins.push(null);   // history-only, exactly like a steer
...
// WHY: specialist work is bounded by its narrow tool set, parent-managed
    ...
    // One repair is an ordinary next loop iteration, not an out-of-band provider
    // call. WHY turn-scoped: a later user request gets its own fresh opportunity.
    let planRepairUsed = false;
    ...
    try {
      ...
      turnLoop: while (true) {
        ...
        // WHY unconditional now (spending rework stage 1, design §3): plan
        // children compact exactly like any other session; there is no
        // unreserved-request exclusion left.
        ...
        let step: StepResult;
        // WHY no plan-gate branch any more (spending rework stage 1, design
        ...
        // like the chip does — no separate bounded/reserved/settled driver.
        {
          ...
          while (true) {
            try {
              step = await withChatGptRequest(this.opts.sessionId, this.opts.isSpecialistChild ? 'specialist' : 'chat', () => this.withRetry(async () => {
                try {
                  ...
                  if (failedPlans.length > 0) {
                    ...
                    if (this.lastAttempt !== undefined) {
                      if (partialAssistantText) this.capture.acceptAttemptText(this.lastAttempt);
                      else this.capture.abandonAttempt(this.lastAttempt);
                    }
                    partialAssistantText = '';
                  }
                  throw err;
        ...
        if (step.interrupted || this.interrupted || this.abort.signal.aborted) {
          // A started plan input already has a visible ordinary tool-use shell.
          ...
          if (canceledPlans.length > 0) {
            ...
            partialAssistantText = '';
          } else if (step.text && step.text.trim().length > 0) {
        ...
        // WHY no soft-limit-overshoot stop here any more (spending rework
        // stage 1, design §3): "The crossing reply's tools still run; only
        // the NEXT request is refused" — a behavior change from the old
        // "nothing it asked for runs". T2's `planSpend.beforeRequest()`
        // (called at the top of this loop) is where a plan's spend-limit
        // crossing now ends the turn, with `stopReason = 'plan_limit_reached'`
        // — not wired here.
        ...
        if (stepHasText || step.toolCalls.length > 0) {
          ...
          const response = sdkShaped
            ? (shellsInHistory ? [...step.responseMessages, this.assistantMessage('', openShells)] : step.responseMessages)
            : [this.assistantMessage(step.text, shellsInHistory ? [...openShells, ...step.toolCalls] : step.toolCalls)];
          ...
          // The shells' own tool-use uuids were recorded at tool-input-start,
          // independently of this position, so a precise combined origin isn't
          // available here — fail closed (null), same rule as the multi-message
          // SDK-response case above.
        ...
        if (isEmptyStep && orderlyFinish) {
          ...
          // WHY no plan-child exclusion any more (spending rework stage 1,
          // design §3): plan children get the ordinary empty-step re-run too.
        ...
        if (step.toolCalls.length === 0) {
          // A plan-input stream that ended without a completed tool-call is not
          ...
          if (openShells.length > 0) {
            ...
            // Each closed call's own tool-result uuid was already recorded by
            // terminateWritingPlans, independently of this position — fail closed
            // (null) rather than state an incomplete origin.
            this.history.push({ role: 'tool', content: calls.map((call) => this.toolResultPart(call, text)) });
            this.historyOrigins.push(null);
          }
        ...
        // A completed call supersedes its stream-start shell by stable id. Keep
        // only non-plan entries in the generic preparing withdrawal path: plan
        // writing is replaced below by the ordinary tool-use/result lifecycle.
        step.pendingPreparing = step.pendingPreparing.filter((prep) => prep.toolName !== 'propose_plan');
        ...
        for (const call of step.toolCalls) {
          ...
          const uuid = this.emitEvent('tool-use', {
            toolUseId: call.toolCallId, toolName: call.toolName, toolInput: call.input,
        ...
        // A closed shell's own tool-result event was already recorded by
        // terminateWritingPlans (capture.recordEvent), independently of this
        // resultOrigins array — so this combined tool message's origin can't be
        // stated precisely once one lands in it. Fail closed (null) rather than
        // publish a checkpoint that omits a real uuid, same rule the multi-message
        // SDK-response case below uses.
        let openShellsClosedHere = false;
        if (openShells.length > 0) {
          ...
          if (shellsInHistory) {
            for (const call of closed) resultParts.push(this.toolResultPart(call, text));
            if (closed.length > 0) openShellsClosedHere = true;
          }
        }
        ...
        for (let i = 0; i < step.toolCalls.length; i++) {
          ...
          if (payload === 'interrupted') {
            ...
            this.historyOrigins.push(openShellsClosedHere ? null : [...resultOrigins]);
            this.emitEvent('user-interrupt', abandonedTurnUsage());
            return;
          }
          ...
          if (payload.planRepairExhausted) {
            // Exactly one repair is safe only inside this established agent loop.
            // WHY stop here: a third provider call would exceed that contract.
            // Pair every sibling the model already emitted, but execute none of
            // them after repair exhaustion — termination means termination even
            // on a provider capable of parallel tool calls.
            stopReason = 'plan_repair_exhausted';
            for (let j = i + 1; j < step.toolCalls.length; j++) {
              const rem = step.toolCalls[j];
        ...
        this.history.push({ role: 'tool', content: resultParts });
        this.historyOrigins.push(openShellsClosedHere ? null : [...resultOrigins]);
        if (stopReason === 'plan_repair_exhausted') break turnLoop;
      ...
      // assistant message live that a resume would then not reproduce. When a
      // writing plan was paired above, ITS assistant message already carries
      // this same partialAssistantText — push a plain one only when there was
      // no plan to carry it.
      if (terminalPlans.length === 0 && partialAssistantText.trim()) {
      ...
      if (this.lastAttempt !== undefined) {
        if (partialAssistantText.trim()) this.capture.acceptAttemptText(this.lastAttempt);
        else this.capture.abandonAttempt(this.lastAttempt);
      }
      // Merge note: the branch's writing-plan pairing (above) decides what
      // reaches history; master's abandoned-turn usage and errorCode ride on
      // the terminal event, so a turn that died mid-proposal still reports
      // what it spent and why.
      if (interrupted) {
        this.emitEvent('user-interrupt', abandonedTurnUsage());
      } else {
  ...
  // WHY notifyPlanStop/planWireMessages/planSetupRequest/planNextRequestBound/
  // runPlanStep are ALL GONE (spending rework stage 1, design §1/§3): the
  // whole bound → durably-reserve → single-transmission → settle driver
  // they formed is deleted along with plan-child mode and budget-adapter.ts.
  // Plan children now go through `consumeStep`/`runStreamOnce` below like any
  // other specialist; T2 attaches `planSpend.beforeRequest()`/`afterReply()`
  // around the ordinary request instead (design §3) — not wired here.
  ...
  private async runStreamOnce(
    ...
    const armWatchdog = () => {
      ...
      stageTimer = setTimeout(() => {
        ...
        stageTimer = setTimeout(() => {
          ...
          // not attempted here.
          // WHY no separate planChild check any more (spending rework stage
          // 1): plan children are specialist children too — buildSpecialistSession
          // (native-session-host.ts) sets isSpecialistChild:true unconditionally
          // for every specialist, plan or not — so `!isSpecialistChild` alone
          // already excludes them from ever parking.
      ...
      while (true) {
        ...
        switch (part.type) {
          ...
          case 'tool-input-start': {
            ...
            if (prepName === 'propose_plan') {
              // Unlike ordinary preparing cards, a plan has a stable product
    ...
    if (interrupted || this.interrupted || abortSignal.aborted) {
      ...
      return { text: assistantText, toolCalls, responseMessages: [], usage: { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheCreationTokens: 0 }, finishReason: undefined, interrupted: true, generationMs: firstChunkAt ? Date.now() - firstChunkAt : 0, pendingPreparing: [], attempt, writingPlanIds: [...preparing].filter(([, entry]) => entry.toolName === 'propose_plan').map(([id]) => id) };
    }
    ...
    return {
      ...
      responseMessages,
      pendingPreparing,
      writingPlanIds: [...preparing].filter(([, entry]) => entry.toolName === 'propose_plan').map(([id]) => id),
  ...
  private async runOneTool(call: ToolCall, recentCalls: string[], planRepairUsed = false): Promise<ToolResultPayload | 'interrupted' | EndTurnResult> {
    const tool = this.toolByName.get(call.toolName);
    ...
    // ahead of every other check including the floors below: asking before
    // the proposal exists would duplicate and invert that flow. Task 9b: a
    // recommendation only changes the card's suggested button; the user's
    // own press is the consent.
    const isPlanProposal = call.toolName === 'propose_plan' || call.toolName === 'recommend_plan_action';
    const configured: PermissionDecision = isPlanProposal
    ...
    return planRepairUsed
      ? {
          ...payload,
          text: `${payload.text}\nPlan repair exhausted; stop and wait for the user's next message.`,
          plan: terminal,
          planRepairExhausted: true,
        }
      : {
          ...payload,
          text: `${payload.text}\nYou have exactly one plan-specific repair opportunity. Call propose_plan once more with corrected arguments.`,
          plan: terminal,
        };
  }
