...
export function projectPlan(plan: PlanRecord, now: number = Date.now()): PlanView {
  ...
  const row = (step: PlanStepV1, kind: PlanStepView['kind'], multiplier: number): PlanStepView => {
    ...
    // the earlier step(s) whose reports the executor feeds in as this step's
    // input (dependencyReports/multiDependencyReports). It is the only place
    // the flow between steps is written down, and the card had never been
    // given it, so the reader could not see how one step fed the next. Passed
    // as the document's own step id(s) — decision 39 widened `of` to an array
    // so a combine step can name every earlier step it needs — turning that
    // into "step 2" or "steps 1, 2 and 3" is the card's job, because only the
    // card knows which rows it drew.
