...
export class NativeSessionHost extends EventEmitter {
   ...
   *  via opts.profile). */
  private toolWiring(
    sessionId: string, cwd: string, preset: ResolvedPreset, profile: CapabilityProfile, gitSnapshot: string, triggers: TriggerIndex,
    ...
    try {
      ...
      session = new HarnessSession(
        { sessionId: opts.sessionId, cwd: opts.cwd, harness, binding: opts.binding, contextLength, profile, pricing, free,
          ...
          ...this.toolWiring(opts.sessionId, opts.cwd, preset, profile, gitSnapshot, triggers, skillEntries, availability.kind === 'resolved' ? availability.skillCatalogIds : undefined) },
        this.modelFactory,
      );
    } catch (err) {
    ...
    try {
      ...
      session = new HarnessSession(
        ...
        { sessionId, cwd, harness, binding, contextLength, profile, pricing, free,
          ...
          ...(mcpServers ? { mcpServers } : {}),
          ...this.toolWiring(sessionId, cwd, preset, profile, gitSnapshot, triggers, skillEntries, skillAllow) },
        this.modelFactory,
      );
