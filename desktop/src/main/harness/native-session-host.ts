...
import type { TranscriptEvent, NativeSendResult, NativeSwitchResult, NativeSwitchFailure, SpecialistsEvent, HookEvent, DelegatedModelsView, SpecialistRunView, ShellEvent, ShellRunView, InjectedMeta, SessionContext, SessionContextText, PlanView } from '../../shared/types';
...
export class NativeSessionHost extends EventEmitter {
  ...
  constructor(
    ...
    private providerTypeFor: (binding: ModelBinding) => Promise<ProfileProviderType | { type: ProfileProviderType; baseUrl?: string } | null>,
  ...
  private async resolveContextAndProfile(binding: ModelBinding): Promise<{ contextLength: number | null; totalSlots: number | null; profile: CapabilityProfile; pricing: ModelPricing | null; free: boolean; providerType: ProfileProviderType; providerBaseUrl?: string; slotsUnknown: boolean }> {
    ...
    const provider = await this.providerTypeFor(binding);
    const type = (typeof provider === 'string' ? provider : provider?.type) ?? 'openrouter'; // unknown → cloud-safe default
    const providerBaseUrl = typeof provider === 'object' && provider !== null ? provider.baseUrl : undefined;
  ...
  private async createInner(opts: CreateNativeSessionOpts): Promise<void> {
    ...
    try {
      ...
      session = new HarnessSession(
        { sessionId: opts.sessionId, cwd: opts.cwd, harness, binding: opts.binding, providerType, providerBaseUrl, contextLength, profile, pricing, free,
  ...
  private buildSpecialistSession(
    ...
    // Required, NOT optional: assembleSystemPrompt falls back to a synchronous
    // git shell-out when it is absent, and every construction site here is a
...
// WHY: specialist work is bounded by its narrow tool set, parent-managed
  ...
  private async resumeInner(sessionId: string, cwd: string, bindingOverride?: ModelBinding): Promise<boolean> {
    ...
    try {
      ...
      session = new HarnessSession(
        ...
        { sessionId, cwd, harness, binding, providerType, providerBaseUrl, contextLength, profile, pricing, free,
  ...
  getHistory(sessionId: string): TranscriptEvent[] | null {
    ...
    if (plan.children.length === 0) return parentEvents;
    const children = plan.children.map((c) => ({ record: c.record, events: this.store.readEvents(c.record.childId, c.cwd) }));
  ...
  async getHistoryAsync(sessionId: string): Promise<TranscriptEvent[] | null> {
    ...
    if (plan.children.length === 0) return parentEvents;
    const children = await Promise.all(plan.children.map(async (c) => ({ record: c.record, events: await this.store.readEventsAsync(c.record.childId, c.cwd) })));
    return mergeChildEvents(sessionId, parentEvents, children);
  }

  /** The one decision both getHistory forms share: WHICH files make up this
   *  session's history — never the reading of them, so the sync and async
   *  forms can never splice different children.
   *
   *  null for a non-live id. `children` is empty when there is no ledger or it
   *  could not be read (logged, replay degrades to the parent's own events — a
   *  broken ledger read must never break replay).
   *
   *  Task 5a: a PLAN specialist has no delegation-ledger row, so its row's past
   *  activity comes from the plan journal instead — same splice, same stamps as
   *  live (parentAgentToolUseId = the propose_plan call, agentId = the
   *  specialist). childTranscriptSources never throws (a damaged journal yields
   *  none), and each source carries its OWN cwd, which is why the record and the
   *  directory travel together here rather than one `workDir` for the lot. */
  private historyPlan(sessionId: string): {
    cwd: string;
    children: Array<{ record: Pick<DelegationRecord, 'parentToolCallId' | 'childId'>; cwd: string }>;
  } | null {
    const entry = this.live.get(sessionId);
    ...
    return {
      cwd: entry.cwd,
      children: [
        ...records.map((record) => ({ record, cwd: record.workDir })),
        ...planChildren.map((c) => ({ record: { parentToolCallId: c.parentToolCallId, childId: c.childId }, cwd: c.cwd })),
      ],
    };
  }
    ...
    try {
      ...
      if (input.resumeChildId) {
        ...
        const session = this.buildSpecialistSession(
          ...
          resolvePreset(this.presetIdFor.get(input.parentId)), parent, gitSnapshot, { plan },
        );
