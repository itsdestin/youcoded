...
export function childAskRouter(deps: ChildAskRouterDeps): NonNullable<HarnessSessionOpts['askUser']> {
  return async (req: AskRequest): Promise<AskDecision> => {
    ...
    // or the ask is canceled (parent interrupt, child teardown). Merge note:
    // master removed the 5-minute hold for EVERY specialist; a plan's
    // specialist waits the same way — its plan identity just rides along so
    // the renderer files the ask under the right plan row.
