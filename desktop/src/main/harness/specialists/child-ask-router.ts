...
export function childAskRouter(deps: ChildAskRouterDeps): NonNullable<HarnessSessionOpts['askUser']> {
  return async (req: AskRequest): Promise<AskDecision> => {
    ...
    // `noAlwaysAllow` (the removal-target floor): the card offered no grant, and
    // one stored anyway could never skip the floor — so nothing is remembered.
    if (decision.behavior === 'allow' && decision.always && !req.noAlwaysAllow && !BUDGET_ASK_TOOL_NAMES.has(req.toolName)) {
