      ...
      if (!authResolved) {
        ...
        if (msg.type === 'auth:ok') {
          ...
          if (!preservePlatform) {
            ...
            // answer, exactly as before. The test is `(pointer: coarse)`, the PRIMARY pointer
            // only, so a touchscreen laptop (primary pointer "fine") reads as mouse-first here
            // and is unchanged too.
