 ...
 * SESSION_RENAMED broadcast to ride — SessionRenameDialog dispatches this
 * event on a successful save, and the row updates without a refetch.
 *
...
export function useRenamedSessions(sources: Record<string, string>) {
  ...
  // authority. Once that snapshot advances — the next browse, or a
  // SESSION_RENAMED broadcast — its own name wins.
