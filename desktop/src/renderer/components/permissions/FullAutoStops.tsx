...
// same approved blue; sharing the class also avoids adding design-lint noise.
export const BROAD_ALLOW_COLORS = 'bg-blue-600/60 hover:bg-blue-600/80 text-blue-100';
...
export function FullAutoStops({ kind, confirmingExternal, toolName, command, floorStop, suppressAlwaysAllow, specialistName, folderName,
  ...
  // The first broad-grant choice is blue; its final confirmation stays green.
  ...
  const alwaysAllowClass = `px-3 ${pad} text-xs font-medium rounded-lg ${BROAD_ALLOW_COLORS} transition-colors disabled:opacity-50 ${focusIdx === 2 ? ring : ''}`;
  ...
  return (
    <div className="px-3 py-2 space-y-2 border-t" style={{ background: fa.bg, borderColor: fa.border }}>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {/* A floor is enforced below remembered rules: do not promise a grant
