...
interface BridgeListeners {
  ...
  /** Specialists plans (Task 6): one plan card's record changed. Pinned here
   *  like specialistEvent because BOTH bridges carry it — preload over
   *  `plans:event` IPC, remote-shim over the same WS message type. */
  planEvent(cb: (e: any) => void): () => void;
