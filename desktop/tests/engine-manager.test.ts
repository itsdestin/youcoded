// EngineManager — the main-process owner of the local engine: install and
// auto-update, backends and devices, downloads, the context window it reports.
// Its sibling tests/engine-manager-router.test.ts drives the manager against a
// faked llama-server spawn; this file uses the real child_process, so the two
// cannot share a file (a file-wide vi.mock would replace spawn here too).
...
import {
  ...
  smallestCompleteModel, routerErrorText, clampContextWindow, resolveEffectiveContext,
...
//
// Found dogfooding. Plan C's headline claim is that a local model's
...
// Everything below plants files in a temp home, so each section owns its
// temp directories and hooks inside its describe.
describe('EngineManager — on a temp home', () => {
  let root: string;
  let userData: string;
  let home: NativeHome;
