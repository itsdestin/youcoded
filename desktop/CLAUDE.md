...
3. **InputBar** (optimistic) — `USER_PROMPT` reducer action dispatched immediately when user sends a message, before the transcript watcher catches up. Dedup uses a `pending` flag on user timeline entries: `USER_PROMPT` appends with `pending: true`, and `TRANSCRIPT_USER_MESSAGE` finds the oldest matching pending entry, clears the flag and moves it to where the transcript recorded it (if no pending match exists, a new `pending: false` entry is appended). Pending bubbles always stay the timeline's tail — every other append goes above them (`appendAbovePending`) — so the sending device and every watching device show the same order (pinned by `tests/chat-order-sender-matches-receiver.test.ts`). This replaces the prior last-10-entries content match, which silently dropped legitimate rapid-fire duplicates. See `docs/PITFALLS.md → Chat Reducer` and `docs/transcript-watcher-spec.md` Design Decision #5.
...
- `npm run typecheck` — Type-checks `src/` and `tests/` with tsgo (TypeScript 7's native compiler), ~3s for both. The build still compiles with `tsc` (TypeScript 6); `tsconfig.json` uses `"bundler"` resolution because TypeScript 7 removed `"node"`.
- `npm run lint` — oxlint (replaced ESLint 2026-09-14, same rules, <1s). A **bug** gate, not a style gate: it carries only rules that catch real defects (conditional React hooks, floating promises in the main process, impossible comparisons), because tsc + vitest + knip were all green while exactly those classes shipped. Every enabled rule is at zero on the tree, so a failure means a NEW defect. The rules that are measured-but-not-yet-enabled — and what each would cost to adopt — are listed at the bottom of `desktop/.oxlintrc.json`; drive one to zero in the same commit that enables it.
- `npm run lint:design` — the design-system check (`@shadcn/lint` through oxlint, config `desktop/.oxlintrc.design.json`): raw palette colors, arbitrary values like `text-[12.5px]`, restyled `components/ui/` primitives, class names Tailwind can't generate. **Report-only** (warnings, exit 0) until each rule's count reaches zero; its messages name the color, size or primitive to use instead. When editing renderer UI, run it on the files you touched and don't add new warnings.
...
- **Persistence:** the source of truth is `~/.claude/youcoded-appearance.json` (`appearance:get`/`appearance:set` in `ipc-handlers.ts`; `persistAppearance()` in `theme-context.tsx`) — active theme slug, theme cycle, `lookOverrides` (the user's GLOBAL layout / glass / bubble / roundness choices laid over every theme; a chosen layout also brings its matching message box — `themes/look-overrides.ts`; absent field = the theme's own; the old per-theme `glassOverrides` were retired 2026-09-24 and are no longer read), display toggles. `localStorage` keys (`youcoded-theme`, …) are only a mirror for the pre-React anti-flash read. Community and user themes live at `~/.claude/wecoded-themes/<slug>/manifest.json` with assets beside it. Anything outside the app that must follow the applied theme reads the JSON file, not localStorage.
...
| **Enter** | Permission/question/prompt card visible, no field focused | Press the highlighted button — it starts on **Yes**, never Always Allow. Only cards in the chat on screen listen (`state/card-keys-context.ts`), and an Enter InputBar already sent as a message is ignored |
